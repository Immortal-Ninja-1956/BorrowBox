...
- **As a Seller**, I want the app to generate a UPI QR code for the exact amount only *after* I physically hand over the item so that the buyer can pay me securely without disputes.
...
*(Note: tRPC abstracts traditional REST endpoints into strongly-typed procedure calls)*
...
- **No Direct Contact Info Leakage:** WhatsApp links and UPI IDs are only exposed *after* a deal is initiated and only to the specific counterparty involved in the deal.
